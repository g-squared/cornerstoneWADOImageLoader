var cornerstoneWADOImageLoader = (function ($, cornerstone, cornerstoneWADOImageLoader) {
    ...
    function extractStoredPixels(dataSet, byteArray, width, height, frame)
    {
        ...
        if( transferSyntax === '1.2.840.10008.1.2.1' ) {  // uncompressed
        ...
        } else if( transferSyntax === '1.2.840.10008.1.2.5' ) { // RLE
            var pixelFormat = getPixelFormat(dataSet);
            var frameData = dicomParser.readEncapsulatedPixelData( dataSet, frame );
            return cornerstoneWADOImageLoader.unRLE( pixelFormat, 1, frameData, width, height);
            
        } else if( transferSyntax === '1.2.840.10008.1.2.4.50' ) { // JPEG 8 bit
            var frameData = dicomParser.readEncapsulatedPixelData( dataSet, frame );
            var jpeg = new JpegImage();
            jpeg.parse( frameData );
            return jpeg.getData(width, height);
           
        } else if( transferSyntax === '1.2.840.10008.1.2.4.51' ) { // JPEG 12 bit
            var frameData = dicomParser.readEncapsulatedPixelData( dataSet, frame );
            var jpeg = new JpegImage();
            jpeg.parse( frameData );
        ...
        throw "no codec for trasferSyntax " + transferSyntax;
    }
    ...
    function makeGrayscaleImage(imageId, dataSet, byteArray, photometricInterpretation, frame, ovls) {
